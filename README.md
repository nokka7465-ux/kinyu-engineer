...
| カテゴリ | 該当記事 | 件数 | タグ色 |
|---|---|---|---|
| 体験談 | article1, 2, 6, 8, 10, 11, 16, 20, 21, 24 | 10 | オレンジ (`--accent2` #C4572A) |
| ノウハウ | article3, 7, 9, 13, 14, 18, 22 | 7 | 深緑 (`--accent` #1A3A2A) |
| 比較 | article4, 12, 17, 23 | 4 | ゴールド (`--accent3` #8B6914) |
| データ | article5, 15, 19, 25 | 4 | グレー (`--text2`) |
| **合計** | | **25** | |
...
- 各記事の `<style>` ブロックは独立しているため、共通スタイルを変更する場合は **全 27 ファイル**（index + privacy + article1〜25）に反映が必要です。
